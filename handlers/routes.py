...
def configure_routes(app):
    ...
    app.config['MYSQL_HOST'] = "40.85.141.46"
    ...
    def User_Details():            
        if "user" in session:            
            ...
            cur.close()            
            return render_template("UserDetails.html",values=users)            
        else:            
    ...
    def Login():
        if request.method == "POST":
            ...
            cur = mysql.connection.cursor()
            found_user = cur.execute("select * from userdtls where name = %s", session["user"])
            cur.close()                               
            if found_user > 0:       
